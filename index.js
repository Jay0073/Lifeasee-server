...
wss.on("connection", (ws) => {
  ...
  const startRecognitionStream = () => {
      console.log(`[${ws.clientId || 'New Client'}] Starting Google Speech recognize stream.`);
      recognizeStream = speechClient
        ...
        .on("error", (err) => {
          ...
          if (ws.readyState === WebSocket.OPEN) {
              ws.send(JSON.stringify({ error: `Google Speech API Error: ${err.message}` }));
          }
        })
        .on("data", (data) => {
          if (data.results[0]?.alternatives[0]) {
            ...
            if (ws.readyState === WebSocket.OPEN) {
                ...
                console.warn(`[${ws.clientId}] WebSocket closed, cannot send transcription.`);
 ...
 const stopRecognitionStream = () => {
  ...
  ws.on("message", (message) => {
    ...
    try {
        ...
        console.error(`[${ws.clientId}] Received non-JSON message or invalid JSON. Ignoring.`);
    ...
    if (parsedMessage.type === "audio_chunk" && parsedMessage.data) {
        ...
        if (!clientData || !clientData.recognizeStream) {
             console.error(`[${ws.clientId}] Failed to get or create recognizeStream. Cannot process audio.`);
             return;
         }
        ...
        try {
            ...
            console.log(`[${ws.clientId}] Received audio chunk (${audioBuffer.length} bytes). Converting...`);
            ...
            // Kill previous ffmpeg process if it exists and hasn't finished
             if (clientData.ffmpegProcess) {
                 console.log(`[${ws.clientId}] Killing previous ffmpeg process for new chunk.`);
                 clientData.ffmpegProcess.kill('SIGKILL');
             }

            // Use ffmpeg to convert WAV buffer to raw PCM (s16le)
            const ffmpegCommand = ffmpeg(inputStream)
                .inputFormat('wav') // Specify input is WAV
                ...
                .on("error", (err, stdout, stderr) => {
                 ...
                 .on("start", (commandLine) => {
                   console.log(`[${ws.clientId}] Started ffmpeg process: ${commandLine}`);
                 })
                .on("end", () => {
         ...
         console.log(`[${ws.clientId}] Received end_audio signal. Client stopped sending.`);
        ...
        console.log(`[${ws.clientId}] Received unknown message type:`, parsedMessage.type);
    }
  });

  // --- Handle Client Disconnection ---
  ws.on("close", (code, reason) => {
    console.log(`[${ws.clientId || 'Client'}] WebSocket connection closed. Code: ${code}, Reason: ${reason ? reason.toString() : 'N/A'}`);
